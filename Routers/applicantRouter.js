...
applicantRouter.get('/accept/:id',expressAsyncHandler (async(req, res) =>{
  ...
  // res.send({mentor:newMentor,message:"approval done"})

  const newMentor=await mentor.save()
  client.messages.create({
    body:'Congratulations, You have been approved as Mentor',
    to: '+91'+newMentor.phoneNumber ,
    from: '+14242383604'
  }).then((message)=>{
...
applicantRouter.post('/register', expressAsyncHandler(async(req, res) => {
    ...
    res.send({
        ...
        token: generateToken(createdApplicant),
