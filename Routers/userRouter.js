...
userRouter.put(
  ...
  userRouter.get(
    ...
    expressAsyncHandler(async(req,res)=>{
      ...
      console.log("inside list router")
  ...
  userRouter.post('/choose/:s_id',expressAsyncHandler(async (req, res) =>{
    const userId = JSON.parse(req.params.s_id);
    
    const user = await User.findById(JSON.parse(req.params.s_id));
    console.log(user)

    const mentorId = req.body.m_id;

    console.log("m_id ",mentorId)
    console.log(mentorId)
    console.log(userId)
    const mentor = await Mentor.findById(req.body.m_id);
    console.log(mentor)
